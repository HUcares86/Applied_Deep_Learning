...
def tokenizeQuestion(data):
    ...
    questions_tokenized = tokenizer(question, add_special_tokens=False)
...
def oneEpochTrain(model, scheduler, optimizer, train_loader):
    ...
    for batch in bar:
        ...
        output = model(input_ids=batch['input_ids'], token_type_ids=batch['token_type_ids'],attention_mask=batch['attention_mask'], labels=batch['labels'])
...
def saveCKPT(model):
    model_save_dir = "saved_Multi_model_scratch"
