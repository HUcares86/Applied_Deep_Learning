...
def main():
    ...
    #     },
    # ]
    # 確認Adafactor可否這樣用!，sche調整???
    optimizer = Adafactor(model.parameters(), relative_step=False, lr=args.learning_rate)
    ...
    lr_scheduler = get_scheduler(
        ...
        optimizer=optimizer, # optim是否可相容???
    ...
    # 測試!!!
    # print("id test!!!")
    # print(raw_datasets['validation']['id'])
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        eval_bar = tqdm(eval_dataloader, desc="Valid")
        total_preds = []
        total_labels = []
        total_id = []
        ...
        if epoch == args.num_train_epochs - 1:
            ...
            do_predict = False
        # # 出結果csv看一下!
        with open("./predsNo_trianer_test.csv", 'w', encoding='utf-8-sig', newline='') as f:
            writer = csv.writer(f, delimiter=',')
            writer.writerow(['pred'])
            writer.writerow(total_preds)
        print("preds.csv saved!")
        # 思考用原title直接取出，不用自己做的label
        with open("./labelsNo_trianer_test.csv", 'w', encoding='utf-8-sig', newline='') as f2:
        ...
        if do_predict:
            ...
            with jsonlines.open(prediction_file, 'w') as writer:
                for i, title in zip(raw_datasets['validation']['id'], decoded_preds):
                    writer.write({
                        "title": title,
                        "id": i
                    })
        ...
        result = get_rouge(preds=total_preds, refs=total_labels)
    ...
    if args.output_dir is not None:
        ...
        unwrapped_model.save_pretrained(
            os.path.join(args.output_dir, "TestModel"), is_main_process=accelerator.is_main_process, save_function=accelerator.save
        )
        if accelerator.is_main_process:
            tokenizer.save_pretrained(os.path.join(args.output_dir, "TestModel"))
            # if args.push_to_hub:
